...
class MatrixConnector:
    ...
    async def start(self) -> None:
        ...
        if creds_path.exists():
            ...
            logger.info("Matrix: using access token, device_id=%s", self._client.device_id)
        else:
        ...
        try:
            ...
            if user_ids and self._client.olm:
                ...
                if users_to_claim:
                    ...
                    await asyncio.wait_for(self._client.keys_claim(users_to_claim), timeout=15)
    ...
    async def _on_room_message(self, room: MatrixRoom, event: RoomMessageText) -> None:
        ...
        try:
            ...
            logger.exception("Error handling message from %s", msg.sender)
    ...
    async def _on_invite(self, room: MatrixRoom, event: InviteMemberEvent) -> None:
        ...
        if event.state_key == self._settings.user_id and event.membership == "invite":
            logger.info("Matrix: invited to %s by %s, joining...", room.room_id, event.sender)
            await self._client.join(room.room_id)

    async def _on_key_request(self, event: RoomKeyRequest) -> None:
        """Log key requests."""
        logger.info("Matrix: key request from %s device %s", event.sender, event.requesting_device_id)
